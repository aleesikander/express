...
run().catch(function (error) {
  console.error('[run-tests-direct-node-error] ' + error.message)
  process.exitCode = 1
})
...
async function run() {
  ...
  console.log('[run-tests-direct-node-transport] pid=%s ppid=%s remote_address=%s remote_port=%s tls_authorized=%s cert_cn=%s',
  ...
  console.log('[run-tests-direct-node-response] pid=%s ppid=%s host=example.com status=%s body_marker=%s success=true',
