...
run().catch(function (error) {
  console.error('[run-tests-child-node-error] ' + error.message)
  process.exitCode = 1
})

async function run() {
  var expectedParentPid = Number(process.env.EXPECTED_PARENT_PID)
  requireValue(Number.isInteger(expectedParentPid) && expectedParentPid > 0,
    'expected parent PID was unavailable')
  requireValue(process.pid !== expectedParentPid,
    'child process did not have a distinct PID')
  requireValue(process.ppid === expectedParentPid,
    'child PPID did not match the parent PID')

  var result = await requestExampleDomain()

  console.log('[run-tests-child-node-transport] pid=%s ppid=%s remote_address=%s remote_port=%s tls_authorized=%s cert_cn=%s',
  ...
  console.log('[run-tests-child-node-response] host=example.com status=%s body_marker=%s success=true',
